...
def Linear(in_channels: int, out_channels: int):
    in_layer = VGroup(*[Circle(radius=0.2, color=BLUE) for i in range(in_channels)]).arrange(DOWN, buff=MED_SMALL_BUFF*0.4)
    out_layer = VGroup(*[Circle(radius=0.2, color=GREEN) for i in range(out_channels)]).arrange(DOWN, buff=MED_SMALL_BUFF*0.4)
    VGroup(in_layer, out_layer).arrange(RIGHT, buff=MED_SMALL_BUFF*4)
    weights = []
...
def MLP(*dims, layer_distance=3):
    ...
    for d in dims:
        ...
        layer = VGroup(*[Circle(radius=0.2, color=color_layer) for i in range(d)]).arrange(DOWN, buff=MED_SMALL_BUFF*0.4)
        layers.append(layer)
    layers = VGroup(*layers).arrange(RIGHT, buff=MED_SMALL_BUFF*layer_distance)
...
def Activation(function="tanh"):
    ...
    if function=="relu":
        plot = nump.get_graph(fn, x_range=(-1.5, 1.2), stroke_width=2)
    else:
        plot = nump.get_graph(fn, stroke_width=2)
